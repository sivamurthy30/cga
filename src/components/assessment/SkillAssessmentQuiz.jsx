...
import BlogReaderModal from '../features/BlogReaderModal';
...
const SkillAssessmentQuiz = ({ skills, onComplete, onClose }) => {
  ...
  const [forcedHard, setForcedHard] = useState(false);
    ...
    for (const skill of skills) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Use local question bank if backend returns empty or only generic placeholders
          questionsMap[skill] = (fetchedQuestions.length === 0 || hasOnlyGenericSelfAssessment)
  ...
  const handleAnswer = (optionIndex) => {
    ...
    if (!currentQuestion) {
      // Safety: skip to next skill if questions are missing
    ...
    // Dynamic Difficulty: 2 medium correct in <30s → force next 2 hard
  ...
  const calculateScores = async () => {
    ...
    try {
      ...
      // Proof generation failed silently — not critical
    }
                        ...
                        <div className="recommended-resources">
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                            {recommendation.resources.map((resource, i) => (
                              <a 
                                key={i}
                                href={resource.link || 'https://roadmap.sh'}
                                target="_blank"
  ...
  const currentSkill = skills?.[currentSkillIndex];
  const questions = currentSkill ? quizQuestions[currentSkill] : undefined;
  const currentQuestion = questions?.[currentQuestionIndex];

  // Guard: still loading questions or index out of bounds
  if (!currentSkill || !questions || !currentQuestion) {
    return (
      <div className="skill-assessment-container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
    ...
    <div className="skill-assessment-container">
      ...
      <div className="question-card">
        <div className="question-difficulty" data-difficulty={currentQuestion.difficulty}>
          {forcedHard ? '🔥 HARD (Difficulty Boost Active)' : currentQuestion.difficulty.toUpperCase()}
        </div>
        {forcedHard && (
          <div style={{ fontSize: 11, color: '#f59e0b', marginBottom: 8, padding: '4px 10px', background: 'rgba(245,158,11,0.1)', borderRadius: 6, display: 'inline-block' }}>
            ⚡ You answered 2 medium questions correctly in under 30s — difficulty increased!
          </div>
        )}
