...
// ─── Manual Skill Entry (fallback when PDF parsing fails) ────────────────────
const COMMON_SKILLS = [
  "Python", "JavaScript", "TypeScript", "Java", "C", "C++", "C#", "Go",
  "React", "Node.js", "Express", "Django", "Flask", "FastAPI", "Spring Boot",
  "HTML", "CSS", "Next.js", "Vue", "Angular", "Tailwind CSS",
  "SQL", "MySQL", "PostgreSQL", "MongoDB", "Redis", "Firebase",
  "Docker", "Kubernetes", "AWS", "Git", "Linux", "CI/CD",
  "Machine Learning", "Deep Learning", "TensorFlow", "PyTorch", "Pandas", "NumPy",
  "Flutter", "React Native", "Swift", "Kotlin",
  "REST", "GraphQL", "Figma", "Agile", "Scrum",
];

const ManualSkillEntry = ({ selectedRole, onComplete, onSkip }) => {
  ...
  return (
    <div className="manual-skill-entry">
      <h3>Select Your Skills</h3>
      <p>Pick the technologies you already know:</p>
      <div className="skills-grid" style={{ marginBottom: '1.5rem' }}>
        {COMMON_SKILLS.map(skill => (
          <button
            key={skill}
            className={`skill-badge ${selected.includes(skill) ? 'has-skill' : ''}`}
            style={{ cursor: 'pointer', border: selected.includes(skill) ? '2px solid var(--accent-primary)' : '2px solid var(--border-primary)' }}
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' }}>
        ...
        />
        <button className="btn btn-primary" onClick={addCustom} style={{ padding: '0.6rem 1.2rem' }}>
          Add
        </button>
      </div>
      ...
      <div className="next-steps-actions">
        <button className="btn btn-primary btn-large" onClick={() => onComplete(selected)}>
...
const OnboardingFlow = ({ onComplete, currentUser, onLogout, theme, toggleTheme }) => {
  ...
  const handleResumeUpload = async (file) => {
    ...
    try {
      ...
      try {
        ...
        uploadResponse = await fetch('/api/resume/upload', {
          method: 'POST',
        ...
        setError('Backend is not running. Please skip this step and add your skills manually.');
        setIsLoading(false);
        return;
      }
  ...
  if (currentStep === 5) {
    // If resume already uploaded, show analysis (even if 0 skills — show warning + manual entry)
    if (resumeData) {
      const hasSkills = resumeData.skills_found && resumeData.skills_found.length > 0;
      const parseWarning = resumeData.parse_warning || (!hasSkills ? "No skills were automatically detected. This can happen with image-based or scanned PDFs. You can add your skills manually below." : null);
        ...
        <div className="onboarding-container">
          ...
          <div className="onboarding-content resume-analysis-screen">
            <div className="analysis-header">
              <h2>{hasSkills ? "Resume Analysis Complete" : "Resume Uploaded"}</h2>
              <p>Selected role: <strong>{selectedRole?.name}</strong></p>
            </div>

            {parseWarning && (
              <div className="error-banner" style={{ background: 'var(--bg-secondary)', borderColor: 'var(--accent-primary)', color: 'var(--text-primary)' }}>
                <p>⚠️ {parseWarning}</p>
                <button onClick={() => { setResumeData(null); setError(null); }}>×</button>
              </div>
            )}
                ...
                {selectedRole && (
                  <div className="skill-gap-analysis">
                    ...
                    {(() => {
                      ...
                      const hasSkillsList = requiredSkills.filter(skill => 
                      ...
                      const completionPercentage = requiredSkills.length > 0
                        ? Math.round((hasSkillsList.length / requiredSkills.length) * 100)
                        : 0;
                        ...
                        <>
                          <div className="skill-progress">
                            ...
                            <div className="progress-stats">
                              <span className="stat-item"><span className="stat-icon">✅</span>{hasSkillsList.length} skills you have</span>
                              <span className="stat-item"><span className="stat-icon">📚</span>{missingSkills.length} skills to learn</span>
                            </div>
                          </div>

                          {hasSkillsList.length > 0 && (
                            <div className="skills-section">
                              <h4 className="section-title"><span className="title-icon">✅</span>Skills You Have ({hasSkillsList.length})</h4>
                              <div className="skills-grid">
                                {hasSkillsList.map((skill, index) => (
                                  <div key={index} className="skill-badge has-skill">{skill}</div>
                                ))}
                              </div>
                            </div>
                          )}
                          ...
                          {missingSkills.length > 0 && (
                            <div className="skills-section">
                              <h4 className="section-title missing"><span className="title-icon">📚</span>Skills to Learn ({missingSkills.length})</h4>
                              <div className="skills-grid">
            ...
            {/* Manual skill entry when PDF parsing failed */}
            {!hasSkills && (
              <ManualSkillEntry
                selectedRole={selectedRole}
                onComplete={(skills) => handleCompleteWithSkills(skills)}
            ...
            {hasSkills && (
              <div className="next-steps-box">
                ...
                <div className="next-steps-actions">
                  ...
                  <button 
                    className="btn btn-secondary"
                    onClick={() => { setResumeData(null); setAiSuggestedRole(null); setAiMatchPercentage(0); }}
                  >
