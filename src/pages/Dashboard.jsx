...
function navigateTo(hash) {
  if (window.location.hash === hash) {
    // Same hash — dispatch manually so the listener fires
...
const Dashboard = ({ learnerProfile, currentUser, onLogout, onChangeRole, onOpenRoadmap, theme, toggleTheme, isPro, onUpgradeClick }) => {
  ...
  const projects = {
    'Frontend Developer':        'Build a portfolio site with dark mode, CSS Grid layouts, and smooth animations.',
    'Backend Developer':         'Create a REST API with JWT auth, rate limiting, and a PostgreSQL database.',
    'Full Stack Developer':      'Build a full-stack app with user auth, file uploads, and real-time updates.',
    'Data Scientist':            'Build an interactive dashboard with live data, charts, and a regression model.',
    'Machine Learning Engineer': 'Train an image classifier with transfer learning and expose it via a FastAPI endpoint.',
    'DevOps Engineer':           'Set up a CI/CD pipeline with Docker, automated tests, and a Kubernetes deployment.',
    'Mobile Developer':          'Build a cross-platform app with offline support and push notifications.',
    'Security Engineer':         'Implement a secure auth system with OAuth2, 2FA, and a full audit log.'
  };

  return (
    <div className="dashboard-container fade-in">
      <Navigation currentUser={currentUser} onLogout={onLogout} theme={theme} toggleTheme={toggleTheme} currentPage="dashboard" isPro={isPro} onUpgradeClick={onUpgradeClick} />
        ...
        <div className="dashboard-content">
          ...
          <header className="dashboard-hero">
            <div className="hero-text">
              ...
              <p>Working towards <span className="highlight">{learnerProfile?.targetRole || 'developer'}</span></p>
            </div>
          ...
          <section style={{ marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
              {PREMIUM_FEATURES.map(({ icon, label, hash, desc }) => (
                ...
                >
                  <span style={{ fontSize: '1.25rem', marginBottom: '0.25rem' }}>{icon}</span>
                  <span style={{ color: 'var(--text-primary)', fontWeight: 600, fontSize: '0.9rem' }}>{label}</span>
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem', lineHeight: 1.4 }}>{desc}</span>
                </button>
              ))}
            </div>
          </section>
          ...
          <div className="dashboard-grid">
            ...
            <div className="grid-main">
              ...
              <div className="premium-card project-card">
                ...
                <div className="project-actions">
                  <button className="btn-solid" onClick={() => navigateTo('#roadmap')}>Start Project</button>
                  <button className="btn-outline" onClick={() => navigateTo('#advanced-concepts')}>View Details</button>
                </div>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                  ...
                  <div>
                    ...
                    <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                      {['Build 3–5 projects using your target stack', 'Solve 20+ problems on LeetCode or HackerRank', 'Read the official docs for your core technologies', 'Contribute to an open source project'].map((goal, i) => (
            ...
            <div className="grid-sidebar">
              ...
              <div className="sidebar-card">
                <h3 className="card-title flex-align gap-2"><span className="text-xl">👥</span> Community</h3>
                <p className="community-text mt-3">
                  Connect with other developers on the same path. Share what you're building, ask questions, and give feedback.
                </p>
                <button className="btn-text mt-2">Find developers →</button>
              </div>
              ...
              <div className="sidebar-card gradient-glow-card">
                ...
                <p className="text-white opacity-80 mt-2 text-sm leading-relaxed">
                  Go deeper on system design, architecture patterns, and the concepts that separate senior engineers.
                </p>
                <button className="btn-white-glass mt-4 w-full" onClick={() => navigateTo('#advanced-concepts')}>
                  Explore
                </button>
              </div>
            </div>
      ...
      {showRoleModal && (
        <div className="modal-overlay" onClick={() => setShowRoleModal(false)}>
          <motion.div className="modal-content premium-modal" onClick={e => e.stopPropagation()}
            ...
            <p className="modal-subtitle">Choose a new path. Your current progress will be reset.</p>
