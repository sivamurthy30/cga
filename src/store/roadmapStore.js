...
const syncNodeToBackend = async (roadmapId, nodeId, completed) => {
  const token = localStorage.getItem('authToken');
  if (!token) return;
  try {
    await fetch('/api/user/roadmap/progress', {
      method: 'POST',
      headers: {
    ...
    console.warn('Could not sync roadmap progress to backend:', err);
  }
};
